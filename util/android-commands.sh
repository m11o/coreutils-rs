...
snapshot() {
    ...
    command="'\
    mkdir -vp ~/.cargo/bin; yes |\
    echo https://packages-cf.termux.dev/apt/termux-main stable main > /etc/apt/sources.list; \
    pkg install rust binutils openssl tar -y; echo \$? > $probe'"
